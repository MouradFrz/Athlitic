                    ...
                    <div class="filter">
                        <form action="{{ route('user.shop') }}">
                            ...
                            <div class="d-flex mb-3">
                                <input type="text" name="minprice" placeholder="Min" class="custom-input" id=""
                                    style="max-width: 50%">
                                <input type="text" name="maxprice" placeholder="Max" class="custom-input" id=""
                                    style="max-width: 50%">
                            </div>
                            ...
                            @foreach ($products as $product)
                                <div class="product">
                                    ...
                                    <div>
                                        ...
                                        <p><span
                                                class=" @if ($product->promo) old-price @else new-price m-0 @endif">${{ $product->price }}</span>
                                            @if ($product->promo)
                                                <span
                                                    class="new-price">${{ round($product->price - ($product->price * $product->promo) / 100, 2) }}</span>
                                            @endif
                                        <div class="buttons">
                                            <button class="custom-button add-to-cart-btn" data-id="{{ $product->id }}"> <i
